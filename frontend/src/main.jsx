...
function Auth({mode,setRole,go,notify}){return <main className="auth-page"><div className="auth-card"><div className="auth-brand"><span className="brand-mark"><BriefcaseBusiness/></span><h2>{mode==="login"?"Welcome back":"Create your JobSphere account"}</h2><p>{mode==="login"?"Sign in to continue to your workspace.":"Join candidates and recruiters building what’s next."}</p></div>{mode==="register"&&<div className="role-switch"><button className={role==="candidate"?"selected":""} onClick={()=>setRole("candidate")}><UserRound/> Candidate</button><button className={role==="recruiter"?"selected":""} onClick={()=>setRole("recruiter")}><Building2/> Recruiter</button></div>}<label>Email address<input type="email" placeholder="you@example.com"/></label><label>Password<input type="password" placeholder="••••••••"/></label>{mode==="register"&&<label>Confirm password<input type="password" placeholder="••••••••"/></label>}<button className="btn primary full" onClick={()=>{notify(mode==="login"?"Signed in successfully":"Account created successfully");go("dashboard")}}>{mode==="login"?"Sign in":"Create account"}</button><div className="auth-link">{mode==="login"?"New to JobSphere?":"Already have an account?"} <button onClick={()=>go(mode==="login"?"register":"login")}>{mode==="login"?"Create account":"Sign in"}</button></div></div></main>}
function PostJob({go,notify}){return <main className="inner-page narrow"><div className="page-heading"><div><div className="kicker">RECRUITER TOOLS</div><h1>Post a new job</h1><p>Give candidates the information they need to understand the opportunity.</p></div></div><form className="form-card" onSubmit={e=>{e.preventDefault();notify("Job posted successfully");go("dashboard")}}><div className="form-grid"><label>Job title<input required placeholder="e.g. Java Full Stack Developer"/></label><label>Company name<input required placeholder="Company name"/></label><label>Location<input required placeholder="Hyderabad / Remote"/></label><label>Job type<select><option>Full-time</option><option>Part-time</option><option>Contract</option><option>Internship</option></select></label><label>Experience level<select><option>Fresher</option><option>Junior</option><option>Mid-level</option><option>Senior</option></select></label><label>Salary range<input placeholder="₹6–10 LPA"/></label></div><label>Skills required<input placeholder="Java, Spring Boot, SQL, React"/></label><label>Job description<textarea rows="7" placeholder="Describe responsibilities, requirements and what the team offers..."/></label><div className="form-actions"><button type="button" className="btn ghost" onClick={()=>go("dashboard")}>Cancel</button><button className="btn primary">Publish job</button></div></form></main>}
