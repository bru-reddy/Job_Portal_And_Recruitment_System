...
const accepted=apps.filter(a=>a.status==="APPROVED_FOR_REFERRAL").length;
const rejected=apps.filter(a=>a.status==="NOT_SELECTED").length;
...
<Dash value={accepted} label="Approved for referral" icon={<CheckCircle2/>}/>
<Dash value={rejected} label="Not selected" icon={<X/>}/>
</div>
...
<button className="quick-action" onClick={()=>go("recruiterApplicants")}><span className="quick-action-icon"><Users/></span><span className="quick-action-copy"><b>Review applicants</b><small>Review submissions and approve relevant candidates for referral.</small></span><span className="quick-action-arrow"><ArrowRight/></span></button>
</div></section>
...
function RecruiterApplicants({user}){const [apps,setApps]=useState([]),[filter,setFilter]=useState("ALL"),[loading,setLoading]=useState(true);
